...
driver = GraphDatabase.driver("bolt://localhost:7687", auth=(user_name, password), database="yelp")
...
query = "MATCH (n1)-[r:WRITE]->(n2) RETURN (n1), (r), (n2)"
...
for i in range(0, len(data)):
    if user_input in str(data[i]["r"].type) or user_input in str(data[i]["n1"]._properties):
        print(data[i]["n1"]["name"], data[i]["r"].type, data[i]["n2"]["text"])
